...
const OrderDashboard = ({ onClose, orders: propOrders, completeOrder: propCompleteOrder, removeOrder: propRemoveOrder }) => {
  ...
  // 支援 Firestore serverTimestamp 格式
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
        <button onClick={() => setShowCompleted(v => !v)} style={{ marginBottom: 24, background: '#007aff', color: '#fff', border: 'none', borderRadius: 12, padding: '14px 32px', fontSize: 20, fontWeight: 600, display: 'block' }}>
          {showCompleted ? '顯示未完成訂單' : '顯示已完成訂單'}
        </button>
      </div>
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center', justifyContent: 'center' }}>
        {filteredOrders.length === 0 ? (
          ...
          filteredOrders.map((order, idx) => (
            <div key={order.orderId || idx} style={{ border: "2px solid #eee", borderRadius: 18, background: '#fafbfc', padding: 24, minWidth: 180, maxWidth: 340, width: '100%', margin: '0 auto', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'center' }}>
              ...
              {order.complete ? (
                <div style={{ display: 'flex', width: '100%', alignItems: 'center', marginTop: 14, marginBottom: 6 }}>
                  <span style={{ flex: 1, color: '#007aff', fontWeight: 700, fontSize: 18, textAlign: 'center' }}>已完成</span>
                </div>
              ) : (
                <div style={{ display: "flex", gap: 16, marginTop: 14, width: '100%' }}>
                  <button onClick={() => { completeOrder(orders.findIndex(o => o === order)); }} style={{ flex: 1, background: "#e00", color: "#fff", fontSize: 18, fontWeight: 600, border: "none", borderRadius: 12, padding: '14px 0' }}>完成訂單</button>
                </div>
              )}
              <div style={{ display: 'flex', width: '100%', marginTop: 8 }}>
                <button onClick={() => removeOrder(orders.findIndex(o => o === order))} style={{ flex: 1, background: "#888", color: "#fff", fontSize: 16, fontWeight: 600, border: "none", borderRadius: 12, padding: '10px 0' }}>刪除訂單</button>
              </div>
            </div>
          ))
        )}
